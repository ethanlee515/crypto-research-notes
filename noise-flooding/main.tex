...
\begin{definition}[R\'enyi divergence]
	Let $\cP$ and $\cQ$ be distributions over integers, and let $\alpha>1$.
	We define
	\begin{equation}
...
\begin{lemma}
	...
	Let $\cP$ and $\cQ$ be distributions over integers, let $\sigma\in\bbR$, and let $\alpha>1$. We have
	\begin{equation}
...
Before finishing the proof, we need the fact on derivatives of infinite sum.
...
We finally prove \Cref{lemma:KL-renyi} and observe that \Cref{theorem:noise-flooding} follows direction from \Cref{lemma:KL-renyi} and \Cref{lemma:DG-renyi-bound}.

\begin{proof}[Proof of \Cref{lemma:KL-renyi}]
	The result follows from L'H\^opital's rule and direct computation afterwards.
	\begin{align*}
		\lim_{\alpha\rightarrow1}\renyi{\cP}{\cQ}
		&=\lim_{\alpha\rightarrow1}\frac{1}{\alpha-1}
		\ln\parens{\sum_{x\in\bbZ}
		\parens{\Pr\sqparens{\cP=x}}^\alpha
		\parens{\Pr\sqparens{\cQ=x}}^{1-\alpha}}\\
		&=\lim_{\alpha\rightarrow1}\frac{\partial}{\partial\alpha}
		\ln\parens{\sum_{x\in\bbZ}
		\parens{\Pr\sqparens{\cP=x}}^\alpha
		\parens{\Pr\sqparens{\cQ=x}}^{1-\alpha}}\\
		...
		\parens{\Pr\sqparens{\cQ=x}}^{1-\alpha}
		\ln\parens{\frac{\Pr\sqparens{\cP=x}}{\Pr\sqparens{\cQ=x}}}\\
		&=\sum_{x\in\bbZ}\Pr\sqparens{\cP=x}
		\ln\parens{\frac{\Pr\sqparens{\cP=x}}{\Pr\sqparens{\cQ=x}}}\\
		&=\KL{\cP}{\cQ}
	\end{align*}
	Notice that we can exchange the derivative and the infinite sum due to \Cref{theorem:derive-infinite-sum},
	for which all the assumptions are satisfied by substituting in discrete Gaussian density functions.
\end{proof}
